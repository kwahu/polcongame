...
public class PlayerCamera : MonoBehaviour
{
		...
		// The minimum clear radius between the camera and the target
	{
		get
		{
			float fieldOfViewRadius = (optimalDistance / Mathf.Sin (90.0f - camera.transform.GetChild(0).camera.fieldOfView / 2.0f)) * Mathf.Sin (camera.transform.GetChild(0).camera.fieldOfView / 2.0f);
