...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	licznik=0;
		...
		if (!grounded && licznik==1){
		
		
		transform.position=transform.position + camera.main.transform.forward*Time.deltaTime*3;
		transform.position=transform.position - camera.main.transform.up*Time.deltaTime*1;
		velocity.y = Mathf.Max (velocity.y, -3);
		}
		if (!grounded && licznik==2){
		
		
		transform.position=transform.position + camera.main.transform.forward*Time.deltaTime*20;
		}
